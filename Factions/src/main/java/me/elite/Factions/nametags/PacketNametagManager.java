...
public class PacketNametagManager {
    ...
    public void updateNametagFor(Player target, Player viewer) {
        ...
        try {
            ...
            String teamName = plugin.getConfigManager().getTeamPrefix() + target.getName().toLowerCase();
    ...
    private void removePlayerFromTeam(Player viewer, Player target) {
        try {
            String teamName = plugin.getConfigManager().getTeamPrefix() + target.getName().toLowerCase();
            if (teamName.length() > plugin.getConfigManager().getMaxTeamNameLength()) {
                teamName = teamName.substring(0, plugin.getConfigManager().getMaxTeamNameLength());
            }
    ...
    private String getRelationSuffix(Player target, Player viewer) {
        ...
        switch (relation) {
            case ALLY:
                return plugin.getConfigManager().getAllySuffix();
            case TRUCE:
                return plugin.getConfigManager().getTruceSuffix();
            case ENEMY:
                return plugin.getConfigManager().getEnemySuffix();
            default:
                return plugin.getConfigManager().getNeutralSuffix();
        }
    }
    ...
    public void onPlayerJoin(Player player) {
        ...
        }, plugin.getConfigManager().getLoginRefreshDelay());
    }
    ...
    public void onFactionChange(Player player) {
        ...
        }, plugin.getConfigManager().getFactionChangeDelay()); // Use config
    }
    ...
    public void onRelationChange(String faction1, String faction2) {
        ...
        }, plugin.getConfigManager().getRelationChangeDelay()); // Use config
    }
