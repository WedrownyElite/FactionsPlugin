...
public class FactionsPlugin extends JavaPlugin implements Listener {
    ...
    public void onEnable() {
        ...
        if (autoSaveInterval > 0) {
            Bukkit.getScheduler().runTaskTimer(this, () -> {
                ...
                getLogger().info("Auto-saved faction data");
            }, 20L * 60 * autoSaveInterval, 20L * 60 * autoSaveInterval);
        }

        // Start periodic cleanup task for expired relation requests
        long cleanupInterval = getConfigManager().getCleanupIntervalHours() / 1000 * 20; // Convert to ticks
        Bukkit.getScheduler().runTaskTimer(this, () -> {
    ...
    public void onDisable() {
        if (configManager.isSaveOnShutdown()) {
            dataManager.saveFactionData();
        }
